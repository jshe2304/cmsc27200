...
\solution{
    \begin{enumerate}[(a)]
        \item {
            ...
            $ f(n) = \sum_{i=1}^{n} i^k $ \\
            $ 1 \cdot g(n) = \sum_{i=1}^{n} n^k $ \\
            $f(n)$ is $O(g)$

            $f(n) \geq \sum_{i={n/2}}^{n}i^k \geq (\frac{n}{2})^k \cdot \frac{n}{2} = \frac{n^{k+1}}{2^{k+1}}$ \\
            $f(n)$ is $\Omega(n^{k+1})$
        }
        \item {
            $ f = \Theta(g) $ \\
            ...
            $ f(n) = \sum_{i=1}^{n} i^{c\log n} $ \\
            $ 1 \cdot g(n) = \sum_{i=1}^{n} n^{c\log n} $ \\
            $f(n)$ is $O(g)$

            $ f(n) \geq (\int_{1}^{n+1} x^{c\log n} dx = \frac{\left(n+1\right)^{c\log n+1}-1}{c\log n+1} )$ \\
            $f(n)$ is $\Omega(n^{k+1})$
        }
        \item {
            $ f = \Omega(g)$ \\

            $ f(n) = \frac{n (n-1) (n-2) (n-3) (n-4)}{ 5!} $ \\
            $ g(n) = n^{4.5} $ \\
            $f(n)$ has polynomial order of $5 > 4.5$. 
        }
        \item {
            $ f = \Omega(g) $ \\
            ...
            $ f(n) = 3^n $ \\
            $ g(n) = 2^{n+1} - 1 $ \\
            $f(n)$ has a greater exponential base than $g(n)$. 
        }
        \item {
            $ f = O(g) $ \\

            $ f(n) = \sum_{i=1}^{n} \log i $ \\
            $ g(n) = \sum_{i=1}^{n} \log n $ \\
            In $f(n)$, $i \leq n$ and $\log$ is monotone. 
        }
        \item {
            $ f = \Theta(n) $ \\
    ...
    \begin{proof}
        ...
        \begin{enumerate}
            ...
            \item {
                ...
                We will now show that $ \exists d \in \mathbb{R}$ such that $\forall n > 0$, $f(n) \leq d \cdot g(n)$. 

                In the case that $\forall n > 0$ $f(n) \leq c \cdot g(n)$ already, then we can choose $d=c$. 

                In the case that there exists an $n > 0$ such that $f(n) > c \cdot g(n)$, 
                let $n_{max}$ be the value of $n$ such that $\frac{f(n)}{g(n)}$ is maximized. 
                Then $f(n) \leq \frac{f(n_{max})}{g(n_{max})} \cdot g(n)$ for all $n > 0$. 
                If there exists some $n$ at which $f(n) > \frac{f(n_{max})}{g(n_{max})} \cdot g(n)$, 
                then we have $\frac{f(n)}{g(n)} > \frac{f(n_{max})}{g(n_{max})}$, which contradicts our choice of $n_{max}$. 
                
                We can now choose any $d \geq \frac{f(n_{max})}{g(n_{max})}$ to satisfy $f(n) \leq d \cdot g(n)$ for all $n > 0$. 
                
                Therefore, in both cases, there exists some d such that for all $n > 0$, $f(n) \leq \frac{f(n_{max})}{g(n_{max})} \cdot g(n)$
                This statement is equivalent to the L.H.S. 
            }
